...
App 冷启动时在启动页自动执行三阶段检测，全程无需用户操作：

1. **Phase 1** — TCP ping 全部节点（20 并发，3s 超时）
2. **Phase 2** — ping 最快的前 **60** 个节点用 `Libv2ray.measureOutboundDelay` 实测真实转发延迟（12 并发），实测后按延迟重排序
3. **Phase 3** — 实测延迟最小的节点启动完整 Xray core（本地 SOCKS `127.0.0.1:10808`），OkHttp 经代理验证 `https://www.youtube.com/generate_204`，失败则顺移下一节点（最多 3 个）
...
- **测速耗时**：Phase 2 实测 60 节点约 30~45s，全程启动页有进度显示；120s 看门狗兜底。
